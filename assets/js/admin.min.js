...
var _metabox_fields = {
  ...
  init: function(){
    ...
    // repater Field
    $(".lbr-re-toggle").on('click', function() {
    ...
    $('.repeater-sortable').sortable({
      ...
      revert: true,
      cursor: 'move',
      handle: '.lbr_re_sort_handle',
      placeholder: 'lbr_re_sort_highlight'
    });
    //$('.repeater-sortable').sortable( "option", "handle", ".lbr_re_sort_handle" );
  },
  fancySelect: function(){
    if ($().select2){
      $(".lbr-select, .lbr-posts-select, .lbr-tax-select").each(function (){
  ...
  load_color_picker: function(){
    if ($('.lbr-color-iris').length>0)
      $('.lbr-color-iris').wpColorPicker(); 
  },
};
