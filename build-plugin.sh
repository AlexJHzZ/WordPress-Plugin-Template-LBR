...
printf "Nombre del plugin: "
read NAME

printf "Carpeta de destino: "
read FOLDER

printf "Inicializar nuevo repositorio git (y/n): "
read NEWREPO
...
echo "Actualización de archivos de complementos..."
...
cp class-$SLUG-meta-box.php class-$SLUG-meta-box.tmp
sed "s/$DEFAULT_TOKEN/$TOKEN/g" class-$SLUG-meta-box.tmp > class-$SLUG-meta-box.php
rm class-$SLUG-meta-box.tmp

cp class-$SLUG-meta-box.php class-$SLUG-meta-box.tmp
sed "s/$DEFAULT_SLUG/$SLUG/g" class-$SLUG-meta-box.tmp > class-$SLUG-meta-box.php
rm class-$SLUG-meta-box.tmp
